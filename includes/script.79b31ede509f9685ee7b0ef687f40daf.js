...
function buildCalendar() {
	...
	const finishedDiv = document.getElementById('past_calendar');
	const isSidebar = calendar.getAttribute('data-sidebar')=="1";
	outputDiv.innerHTML = "";

	var i = 0; //active events
	var j = 0; //past events
	var show = false;
	calendarData.forEach(event => { //iterate each event
		var isEnded = event.start<yesterday;
		var isThisYear = event.start>startOfYear;
		...
		if(!showPastEvents && isEnded && !isSidebar) show = false; //dont show past events
		if(isSidebar && !isThisYear) show = false;

		if(show) {
			const clone = template.cloneNode(true); //clone the event template from the dom
			clone.id = 'event'+i;
			clone.classList.add(event.filter);
			if(isEnded && isSidebar && finishedDiv) {
				j++;
				finishedDiv.appendChild(clone);
			} else {
				i++;
	...
	if(j==0 && finishedDiv) {
		var noEvents = document.createElement('div');
		noEvents.textContent = "No past events for this year yet.";
		finishedDiv.appendChild(noEvents);
	}
}
